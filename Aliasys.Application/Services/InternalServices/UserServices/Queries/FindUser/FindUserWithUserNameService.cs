...
{
    public class FindUserWithUserNameService : IFindService<ResultFindDto, string>
    {
        private readonly IDataBaseContext _context;
        public FindUserWithUserNameService(IDataBaseContext context)
        {
            _context = context;
        }
        public ResultDto<ResultFindDto> Find(RequestDto<string> request)
        {
            try
            {
                var result = _context.Users.AsNoTracking().FirstOrDefault(p => p.UserName.Trim().ToLower() == request.Parameter.Trim().ToLower());
                if (result != null)
                {
