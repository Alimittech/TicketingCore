...
{
    public class FindUserWithIdService : IFindService<ResultFindDto, int>
    {
        private readonly IDataBaseContext _context;
        public FindUserWithIdService(IDataBaseContext context)
        {
            _context = context;
        }

        public ResultDto<ResultFindDto> Find(RequestDto<int> request)
        {
            try
            {
                var result = _context.Users.AsNoTracking().FirstOrDefault(p => p.Id == request.Parameter);
                if (result != null)
                {
