...
{
    public class FindOrganizationWithIdService : IFindService<ResultFindOrganizationDto, int>
    {
        private readonly IDataBaseContext _context;
        public FindOrganizationWithIdService(IDataBaseContext context)
        {
            _context = context;
        }

        public ResultDto<ResultFindOrganizationDto> Find(RequestDto<int> request)
        {
            try
            {
                var Result = (from o in _context.Organizations.AsNoTracking().Where(p => p.Id == request.Parameter)
